    ...
    def evaluate_tiou(self, tiou):
        ...
        for scorer, method in self.scorers:
            ...
            if method != 'SPICE':
                ...
                if len(tokenize_res) == 0 or len(tokenize_gts) == 0:
                    ...
                    for vid_id in gt_vid_ids:
                        ...
                        score_Object = np.asarray([scores_all_sents[ids[index]]['Object']['f'] for index in vid2capid[vid_id]])
                        score_Relation = np.asarray([scores_all_sents[ids[index]]['Relation']['f'] for index in vid2capid[vid_id]])
                        score_Attribute = np.asarray([scores_all_sents[ids[index]]['Attribute']['f'] for index in vid2capid[vid_id]])
                        all_scores[vid_id] = np.mean(score) if score else 0
                        spice_Object[vid_id] = np.mean(score_Object[~pd.isnull(score_Object)]) if list(score_Object[~pd.isnull(score_Object)]) else 0
                        spice_Relation[vid_id] = np.mean(score_Relation[~pd.isnull(score_Relation)]) if list(score_Relation[~pd.isnull(score_Relation)]) else 0
                        spice_Attribute[vid_id] = np.mean(score_Attribute[~pd.isnull(score_Attribute)]) if list(score_Attribute[~pd.isnull(score_Attribute)]) else 0

                    scores = np.mean(list(all_scores.values()), axis=0)
                    scores_Object = np.mean(list(spice_Object.values()), axis=0)
                    scores_Relation = np.mean(list(spice_Relation.values()), axis=0)
                    scores_Attribute = np.mean(list(spice_Attribute.values()), axis=0)
            ...
            if type(method) == list:
                ...
                output[method + '_Object'] = np.mean(scores_Object)
                output[method + '_Relation'] = np.mean(scores_Relation)
                output[method + '_Attribute'] = np.mean(scores_Attribute)
            else:
                output[method] = np.mean(scores)
                if self.verbose:
